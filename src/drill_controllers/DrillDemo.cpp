...
DrillDemo::DrillDemo(const Drill &drill) :
...
void DrillDemo::handleTime(const std::string &time) {
  args.screen->lblMainStat->setText(time);
}

void DrillDemo::handleTimeStart() {
  spawn();
  ctrlTrack->setEnabled(true);
}

void DrillDemo::handleTimeEnd() {
...
void DrillDemo::handleKilled(const std::shared_ptr<HitTarget> &target) {
...
// @formatter:off
void DrillDemo::setup(const DrillControllerSetupArgs &args) {
  this->args = args;

  args.world->control(ctrlTime = std::make_shared<TimeController>(TimeControllerArgs{
    .durationSeconds = params.duration,
    .handleTime = std::bind(&DrillDemo::handleTime, this, std::placeholders::_1),
    .handleStart = std::bind(&DrillDemo::handleTimeStart, this),
    .handleEnd = std::bind(&DrillDemo::handleTimeEnd, this)
  }));
  ...
  args.world->control(ctrlTrack = std::make_shared<TrackController>(TrackControllerArgs{
    ...
    .handleKilled = std::bind(&DrillDemo::handleKilled, this, std::placeholders::_1),
  }));
...
void DrillDemo::pause() {
  ctrlCam->setEnabled(false);
  ctrlTrack->setEnabled(false);
}

void DrillDemo::resume() {
...
// world controller
//
void DrillDemo::update(double dt) {}

void DrillDemo::handle(const UiEvent &event) {}

//
// controller methods
//
void DrillDemo::spawn() {
